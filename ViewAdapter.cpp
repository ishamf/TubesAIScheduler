...
void ViewAdapter::run_simulated_annealing(const double temp, const double cool, const double loop)
{
    State s = build_state();
    Annealing a(s,temp,cool);
    a.simulatedAnnealing(generator, loop);
    s = a.currentstate;

	update_courses(s.get_courses());
}

void ViewAdapter::run_genetic_algorithm(const int pop, const float cmutate, const float cxover, const int uchange, const int max_iterations)
{
	State s = build_state();

    int last_worst = -1, last_best = -1, unchanged = 0, iters=0;
    ...
    while ((test1.get_alpha().fitness_score() > 0)&&(unchanged < uchange)&&iters<max_iterations) { //50 unchanged state to stop iteration
        test1.selection();
        test1.xover();
        test1.mutation();
        test1.elitist();
